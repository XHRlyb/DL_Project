...
from src.dataset.dataloader import GMDataset, get_dataloader
...
def eval_model(model, classes, bm, last_epoch = True, verbose = False, xls_sheet = None):
    ...
    pred_time = []
    objs = paddle.zeros(len(classes), device = device)
    cluster_acc = []
    cluster_purity = []
    cluster_ri = []
    prediction = []
    ...
    for i, cls in enumerate(classes):
        ...
        pred_time_list = []
        obj_tot_num = paddle.zeros(1, device = device)
        cluster_acc_list = []
        cluster_purity_list = []
        cluster_ri_list = []
        prediction_cls = []
        ...
        for inputs in dataloaders[i]:
            if iter_num >= cfg.EVAL.SAMPLES / inputs['batch_size']:
                break
            if model.module.device != paddle.CPUPlace(): #???
            ...
            with paddle.seg_grad_enabled(False):
                timer.tick()
                outputs = model(inputs)
                pred_time_list.append(paddle.full((batch_num, ), timer.toc() / batch_num))
            ...
            if cfg.PROBLEM.TYPE == '2GM':
                ...
                for b in range(outputs['perm_mat'].shape[0]):
                    ...
                    eval_dict = dict()
                    id_pari = inputs['id_list'][0][b], inputs['id_list'][1][b]
        ...
        if cfg.PROBLEM.TYPE == 'MGM3':
            cluster_acc.append(paddle.concat(cluster_acc_list))
            cluster_purity.append(paddle.concat(cluster_purity_list))
            cluster_ri.append(paddle.concat(cluster_ri_list))
    ...
    if not paddle.any(paddle.isnan(objs)):
        ...
        for idx, (cls, cls_obj) in enumerate(zip(classes, objs)):
            ...
            if xls_sheet: 
                xls_sheet.write(xls_row, idx+1, cls_obj.item())
        print('average objscore = {:.4f}'.format(paddle.mean(objs)))
        if xls_sheet:
            xls_sheet.write(xls_row, idx+2, paddle.mean(objs).item()) 
            xls_row += 1

    if cfg.PROBLEM.TYPE == 'MGM3':
        print('Clustering accuracy')
        if xls_sheet: 
            xls_sheet.write(xls_row, 0, 'cluster acc')
        for idx, (cls, cls_acc) in enumerate(zip(classes, cluster_acc)):
            print('{} = {}'.format(cls, format_metric(cls_acc)))
            if xls_sheet: 
                xls_sheet.write(xls_row, idx+1, paddle.mean(cls_acc).item())
        print('average clustering accuracy = {}'.format(format_metric(paddle.concat(cluster_acc))))
        if xls_sheet:
            xls_sheet.write(xls_row, idx+2, paddle.mean(paddle.concat(cluster_acc)).item()) 
            xls_row += 1

        print('Clustering purity')
        if xls_sheet: 
            xls_sheet.write(xls_row, 0, 'cluster purity')
        for idx, (cls, cls_acc) in enumerate(zip(classes, cluster_purity)):
            print('{} = {}'.format(cls, format_metric(cls_acc)))
            if xls_sheet: 
                xls_sheet.write(xls_row, idx+1, paddle.mean(cls_acc).item()) 
        print('average clustering purity = {}'.format(format_metric(paddle.concat(cluster_purity))))
        if xls_sheet:
            xls_sheet.write(xls_row, idx+2, paddle.mean(paddle.concat(cluster_purity)).item()) 
            xls_row += 1

        print('Clustering rand index')
        if xls_sheet: 
            xls_sheet.write(xls_row, 0, 'rand index')
        for idx, (cls, cls_acc) in enumerate(zip(classes, cluster_ri)):
            print('{} = {}'.format(cls, format_metric(cls_acc)))
            if xls_sheet: 
                xls_sheet.write(xls_row, idx+1, paddle.mean(cls_acc).item()) 
        print('average rand index = {}'.format(format_metric(paddle.concat(cluster_ri))))
        if xls_sheet:
            xls_sheet.write(xls_row, idx+2, paddle.mean(paddle.concat(cluster_ri)).item()) 
            xls_row += 1

    print('Predict time')
    if xls_sheet: 
        xls_sheet.write(xls_row, 0, 'time')
    for idx, (cls, cls_time) in enumerate(zip(classes, pred_time)):
        print('{} = {}'.format(cls, format_metric(cls_time)))
        if xls_sheet: 
            xls_sheet.write(xls_row, idx + 1, paddle.mean(cls_time).item())
    print('average time = {}'.format(format_metric(paddle.concat(pred_time))))
    if xls_sheet:
        xls_sheet.write(xls_row, idx+2, paddle.mean(paddle.concat(pred_time)).item()) 
        xls_row += 1
    
    bm.rm_gt_cache(last_epoch = last_epoch)

    return paddle.Tensor(recalls)

if __name__ == '__main__':
    from src.utils.dup_stdout_manager import DupStdoutFileManager
    from src.utils.parse_args import parse_args
    from src.utils.pring_easydict import print_easydict

    args = parse('Deep learing of graph matching evaluation code.')
