...
sparse_dot = load(name='sparse_dot',
                  sources=['src/extension_pdl/sparse_dot/sparse_dot.cc',
                           'src/extension_pdl/sparse_dot/csr_dot_csc_cuda.cu',
                           'src/extension_pdl/sparse_dot/csr_dot_diag_cuda.cu'],
                  extra_include_paths=[
...
class CSXMatrix3d:
    def __init__(self, inp, shape, device=None):
        ...
        def from_tensors(ind: paddle.Tensor or np.ndarray, indp: paddle.Tensor or np.ndarray,
            ...
            """
            if type(ind) == paddle.Tensor and device is None:
                device = ind.device

            if type(ind) is paddle.Tensor:
                indices_t = paddle.cast(ind, paddle.int64)#.to(device)
            else:
                indices_t = paddle.to_tensor(ind, dtype=paddle.int64, place=device)
            if type(indp) is paddle.Tensor:
                indptr_t = paddle.cast(indp, paddle.int64)#.to(device)
            else:
                indptr_t = paddle.to_tensor(indp, dtype=paddle.int64, place=device)
            if type(data) is paddle.Tensor:
                data_t = paddle.cast(data, data.dtype)#.to(device)
            else:
                data_t = paddle.to_tensor(data, place=device)
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int):
            ...
            assert isinstance(indptr_offset, paddle.Tensor)
            indptr.append(indptr_offset.view(1))

            indices = paddle.cat(indices)
            indptr = paddle.cat(indptr)
            data = paddle.cat(data)
    ...
    @property
    def device(self):
        return paddle.device#self.indices.device
    ...
    def to(self, tgt):
        """
        Compatible to paddle.Tensor.to()
        :param tgt: target, can be paddle.device or paddle.dtype
        :return: a new instance
        """
        if isinstance(tgt, paddle.device):
            return self.__class__([x.to(tgt) for x in [self.indices, self.indptr, self.data]], self.shape)
        elif isinstance(tgt, paddle.dtype):
    ...
    def cuda(self):
        """
        Compatible to paddle.Tensor.cuda()
    ...
    def cpu(self):
        """
        Compatible to paddle.Tensor.cpu()
...
def dot(csr: CSRMatrix3d, csc: CSCMatrix3d, dense=False):
    ...
    if csr.indptr.device == paddle.device('cpu'):
...
def _max(inp, *args, **kwargs):
    ...
    elif type(inp) == paddle.Tensor:
        return paddle.max(inp, *args, **kwargs)
    else:
        raise ValueError('Data type {} not understood.'.format(type(inp)))
