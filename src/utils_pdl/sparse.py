...
def sbmm(t1, t2):
    ...
    """
    return SparseDenseDenseBMM(t1, t2)
...
def sssbmm_diag(m1, m2):
    ...
    This is a wrapper function and does not support gradient.
    """
    if (type(m1) == list and type(m1[0]) == paddle.Tensor) or type(m1) == paddle.Tensor:
        m1 = paddle2ssp(m1)
    if (type(m2) == list and type(m2[0]) == paddle.Tensor) or type(m2) == paddle.Tensor:
        m2 = paddle2ssp(m2)
...
class SparseDenseDenseBMM(paddle.nn.Layer):
    """
...
def bilinear_diag_paddle(s_t1: CSRMatrix3d, d_t2: paddle.Tensor, s_t3: CSCMatrix3d, device=None):
    """
...
def slicing_paddle(s_t, slice, preserve_dim=False):
    ...
    coo = s_t._indices()
    data = s_t._values()
    idx_flag = paddle.ones(coo.shape[1], dtype=paddle.uint8, device=device)
    for i in range(dim):
        s = slice[i]
        if s == -1:
            continue
    ...
    if not preserve_dim:
        ...
        if dim_flag.numel() == 0:
            ...
            coo = coo[:, idx]
            coo = coo[dim_flag, :]
            shape = paddle.Size(paddle.tensor(s_t.shape)[dim_flag])
    else:
        coo = coo[:, idx]
        coo.mul_((slice == -1).type(coo.dtype).view(-1, 1))
        _dtype = paddle.int32
        shape = paddle.Size(paddle.tensor(s_t.shape, dtype=_dtype, device=device) * (slice == -1).type(_dtype)
...
def ssp2paddle(M, batch='dim', dtype=paddle.float32, device=None):
    """
    ...
    if batch == 'list':
        ...
        outp = paddle.sparse_coo_tensor(coo, data, paddle.Size([batch_num] + list(_M.shape)), dtype=dtype, device=device)
...
if __name__ == '__main__':
    t = paddle.tensor([[[ 1,  2,  3,  4],
                       [11, 22, 33, 44]]])
    t = dense_to_sparse(t)
    s = slicing_paddle(t, paddle.tensor((0, 0, 1)), preserve_dim=True)
    print(s.to_dense())


    from paddle.autograd import gradcheck
    input = (dense_to_sparse(paddle.randn(1, 20, 30, dtype=paddle.double, requires_grad=True)),
             paddle.randn(1, 30, 40, dtype=paddle.double, requires_grad=True))
