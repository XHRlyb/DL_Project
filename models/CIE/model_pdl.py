    ...
    def __init__(self):
        ...
        for i in range(self.gnn_layer):
            ...
            if i == self.gnn_layer - 2:  # only second last layer will have cross-graph module
                ...
                self.cross_layer_edge = nn.Linear(cfg.CIE.GNN_FEAT * 2, cfg.CIE.GNN_FEAT)
    ...
    def forward(self, src, tgt, P_src, P_tgt, G_src, G_tgt, H_src, H_tgt, ns_src, ns_tgt, K_G, K_H, type='img'):
        ...
        for i in range(self.gnn_layer):
            ...
            if i == self.gnn_layer - 2:
                ...
                s = s.unsqueeze(1)
                new_emb_edge1 = cross_graph_edge(paddle.concat((emb_edge1, paddle.matmul(paddle.matmul(s, emb_edge2), s.transpose(2, 3))), axis=1).permute(0, 2, 3, 1))
                new_emb_edge2 = cross_graph_edge(paddle.concat((emb_edge2, paddle.matmul(paddle.matmul(s.transpose(2, 3), emb_edge1), s)), axis=1).permute(0, 2, 3, 1))
                emb_edge1 = new_emb_edge1
                emb_edge2 = new_emb_edge2
                '''

        return ss[-1], hungarian(ss[-1], ns_src, ns_tgt)