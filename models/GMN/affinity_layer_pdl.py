...
class Affinity(nn.Layer): 
    ...
    def __init__(self, d):
        ...
        lambda1 = paddle.ParamAttr(initializer=nn.initializer.Assign(paddle.to_tensor(tmp1, dtype='float64')))
        lambda2 = paddle.ParamAttr(initializer=nn.initializer.Assign(paddle.to_tensor(tmp2, dtype='float64')))
        self.lambda1 = self.create_parameter([self.d,self.d], attr = lambda1)
        self.lambda2 = self.create_parameter([self.d,self.d], attr = lambda2)
