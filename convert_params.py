...
def convert_params(model_th, model_pd, model_path):
    ...
    for key_pd in tqdm(state_dict_pd.keys()):
        ...
        elif ('cross_layer_edge' in key_pd) :
            key_th = key_pd.replace('cross_layer_edge', 'cross_graph_edge_0')
...
def CIE_convert():
    ...
    with fluid.dygraph.guard():
        model_th = tchCIE()
        model_pd = pdlCIE()
        #load_model(model_th, "output/vgg16_CIE_voc/params/params_0020.pt")
        load_model(model_th, "pretrained/pretrained_params_vgg16_cie_willow.pt")
        #load_model(model_th , 'share_param.pt')
        model_path = "./pretrained/new_vgg16_cie_willow"
        print(model_th.state_dict().keys())
        print(len(model_th.state_dict().keys()))
        print(model_pd.state_dict().keys())
        print(len(model_pd.state_dict().keys()))
